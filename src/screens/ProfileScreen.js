...
export default function ProfileScreen({ navigation }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (avatarUri) {
        fd.append('profile_picture', { uri: avatarUri, name: 'avatar.jpg', type: 'image/jpeg' });
      }
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setLoading(true);
              await deleteAccount({ role: 'buyer', logout: finishAccountSession, user });
              Alert.alert('Account Deleted', 'Your account has been deleted successfully.', [{ text: 'OK' }]);
            } catch (error) {
        ...
        <View style={styles.avatarWrap}>
          <TouchableOpacity onPress={pickAvatar} style={styles.avatarCircle}>
            {(avatarUri || user?.profile_picture) ? (
              <Image
                source={{
                  uri: avatarUri || (user?.profile_picture?.startsWith('http') ? user.profile_picture : `${client.defaults.baseURL.replace('/api', '')}/storage/${user.profile_picture}`),
                }}
...
const styles = StyleSheet.create({
  ...
  deleteBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#fef2f2', borderWidth: 1, borderColor: '#fca5a5', borderRadius: 12, paddingVertical: 13, marginTop: 12 },
